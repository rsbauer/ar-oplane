...
// 2018-06-11 RSB: Code retrieved from
// https://github.com/fmo91/PluggableApplicationDelegate/blob/master/PluggableApplicationDelegate/Classes/ApplicationServicesManager.swift
// Skipped using it in Cocoapod flavor since it doesn't look like it gets updated
// often and skips the app start pod penalty.
...
open class PluggableApplicationDelegate: UIResponder, UIApplicationDelegate {
	...
	private lazy var localServices: [ApplicationService] = {
		return self.services
	}()
	
	@discardableResult
	private func apply<T, S>
		(_ work: (ApplicationService, @escaping (T) -> Void) -> S?, completionHandler: @escaping ([T]) -> Swift.Void) -> [S] {
	...
	open func applicationDidFinishLaunching(_ application: UIApplication) {
		localServices.forEach { $0.applicationDidFinishLaunching?(application) }
	}
	
	@available(iOS 6.0, *)
	open func application(_ application: UIApplication, willFinishLaunchingWithOptions
		launchOptions: [UIApplication.LaunchOptionsKey: Any]? = nil) -> Bool {
		var result = false
		var functionResult: Bool = false
		
		for service in localServices {
			functionResult = service.application?(application, willFinishLaunchingWithOptions: launchOptions) ?? false
	...
	open func application(_ application: UIApplication, didFinishLaunchingWithOptions
		...
		for service in localServices {
			functionResult = service.application?(application, didFinishLaunchingWithOptions: launchOptions) ?? false
			if functionResult == true {
				result = true
			}
		}
	...
	@available(iOS 2.0, *)
	open func application(_ application: UIApplication, didChangeStatusBarOrientation
	...
	// This callback will be made upon calling -[UIApplication registerUserNotificationSettings:].
	...
	open func application(
		...
		*/
		...
		// 2018-09-08 RSB:
		// Due to a production issue, the code above has been commented out
		// Silent push takes place, this method gets called, then does it's magic
		// the ApplicationServicesManager calls it's completionHandler with the
		// result BEFORE the actual background task has finished and tried
		// returning ITS result.  When it tries to return its result, the app
		// crashes.
		//
		// The below approach is a bit of brute force.  Only ONE service can
		// implement the didReceiveRemoteNotification and return a value.
		//
		// The alternative would be implementing semaphores in this method to
		// force wait until all services have called their completion handler
		for service in localServices {
			service.application?(application, didReceiveRemoteNotification: userInfo, fetchCompletionHandler: completionHandler)
		}
	}
	...
	// background in order to handle the
	...
	// finished handling the callbacks.
	@available(iOS 7.0, *)
	open func application(
		_ application: UIApplication,
	...
	open func application(
		...
		let returns = apply({ (service, restorationHandler) -> Bool in
			service.application?(application, continue: userActivity, restorationHandler: restorationHandler) ?? false
		}, completionHandler: { results in
	...
	// This will be called on the main thread after the user indicates they want to accept a CloudKit sharing
	// invitation in your application.
